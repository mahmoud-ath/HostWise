...
│  · in-process axum backend (app/backend)   │
...
- **Frontend** — Next.js, built with `output: 'export'` into `app/frontend/out/`.
...
- **Backend** — the `app/backend/` crate (axum + sqlx/SQLite) is a **path
...
cd app/frontend && bun run tauri:dev
```

> Web/browser-only dev: `cd app/backend && cargo run` (backend on 8000) in one
> terminal, then `cd app/frontend && bun run dev` in another. Start the backend
...
Artifacts land in `app/frontend/src-tauri/target/release/bundle/`.
...
- **PDF export** is generated by the Rust backend (`app/backend/reports`) with
...
- Version metadata is synced across `app/backend/Cargo.toml`,
