...
Two processes. The browser version of the frontend expects the backend at
**`http://127.0.0.1:8000`** (its hard-coded fallback API base).

```bash
# Terminal 1 — Rust backend → http://127.0.0.1:8000
cd backend-rs
cargo run
...
| `CORS_ORIGINS` | `["http://localhost:3000","tauri://localhost"]` | Allowed origins |
...
**`https://github.com/<owner>/HostWise/releases/tag/v0.7.1`**.
