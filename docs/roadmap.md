...
| --- | --- | --- |
| **Real connectors** (Airbnb/Booking/VRBO APIs, iCal) via `ConnectorRegistry` seam | The product story is "analytics over booking data" — connectors feed it | Removes manual CSV entry; retention |
| **Notifications engine** + report scheduler wired to stored settings (`notify_*`, `report_auto_generate`, `report_send_email`) | Settings already define them | Proactive insights |
...
| ~~Make **import settings** (date format/currency) actually used~~ — ✅ **Done (v2)** — `ConnectorService` honors `import_date_format` + `default_currency`; JSON supported; remaining: encoding/delimiter + column-mapping wizard | Stored but unused | Better import reliability |
